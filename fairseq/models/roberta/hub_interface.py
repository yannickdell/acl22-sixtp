...
class RobertaHubInterface(nn.Module):
    ...
    def extract_features(
        self, tokens: torch.LongTensor, return_all_hiddens: bool = False, extra_params=None,
    ) -> torch.Tensor:
        if tokens.dim() == 1:
    ...
    def predict(self, head: str, tokens: torch.LongTensor, return_logits: bool = False, extra_params=None):
        features = self.extract_features(tokens.to(device=self.device), extra_params = extra_params)
